...
export default async function handler(req, res) {
  try {
   ...
   const response = await axios.get(directionsUrl, { params });
    ...
    const mapUrl = `${staticMapBaseUrl}?size=600x300&path=enc:${polyline}&markers=color:green|${process.env.HOME_ADDRESS}&markers=color:red|${process.env.WORK_ADDRESS}&key=${params.key}`;
